...
class TakeoutClient {
    constructor(token, baseUrl = 'https://takeout.bysourfruit.com') {
        this.token = token
    ...
    */
    ...
    async login(token) {
        ...
        if (res.ok) {console.log(response.message); return {message: response.message, authenticated: true}}
    }
    ...
    */
    async getHTMLFileContents(file) {
        if (typeof 'window' === 'undefined') throw new Error('File uploads only supported in browser')
    ...
    */
    async send(emailTemplate) {
        ...
        const response = await res.json()
        return response["header"]["message-id"]
    }
    ...
    * IT RELIES ON THE API. IF THE API CODE IS FUCKED, SO IS THIS BAREBONES FUNCTION!
    */
